...
프로젝트를 개발할 때, 코드를 수정하고 변경된 실행 결과를 확인하는 것을 계속 반복해야 한다. 하지만 위에서 설명한 필수적인 환경 설정만 따라했다면 코드를 수정한 다음 계속해서 `npm start` 명령을 실행하고 결과를 확인해야해서 여간 불편한게 아니다.. 변경 사항이 있을 때 마다 자동으로 다시 webpack을 실행해주고 결과 화면을 업데이트를 해주는 **webpack-dev-server** 를 이용하면 삶의 질이 200정도 올라가는 것이 학계의 정설.
...
4. `npm build` 하면 브라우저가 열리고, _App.jsx_ 에 있는 hello world 텍스트를 변경하였을 때 실시간으로 변경이 반영 되는 것을 확인한다.
